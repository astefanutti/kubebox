...
class Events {
  constructor ({ screen, client, debug, cancellations, navbar, namespace, status }) {
    ...
    const { promise, cancellation } = client.events(namespace).asTable().get({ cancellable: true });
    ...
    until(promise)
      ...
      .then(_ => {
        const { promise, cancellation } = client.events(namespace).asTable()
    ...
    this.describe_event = function (i) {
      ...
      const { promise, cancellation } = client.event(namespace, name).asYaml().get({ cancellable: true });
      cancellations.add(key, cancellation);
      until(promise)
        ...
        .then(response => {
          ...
          event_description.setContent(highlight(events_description, { language: 'yaml', ignoreIllegals: true, theme: { string: plain } }));
        }).catch(error => {
          debug.log(`{red-fg}Error fetching event '${name}' in namespace '${namespace}'{/red-fg}`);
          event_description.setContent(`{red-fg}${error.toString()}{/red-fg}`);
        });
    }
