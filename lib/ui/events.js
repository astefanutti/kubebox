...
class Events extends EventEmitter {
  ...
  constructor ({ client, debug, status, namespace, object }) {
    super();
    const cancellations = new task.Cancellations();

    const columns = ['Namespace', 'Last Seen', 'Type', 'Reason', 'Object', 'Message'];
    ...
    this.on(NavBar.AddEvent, ({ screen, page }) => {
      ...
      if (object) {
        request = request.fieldSelector(`involvedObject.uid=${object}`);
      }
      ...
      until(promise)
        ...
        .then(response => {
          ...
          const id = setInterval(refreshEventTimes, 1000, screen);
          cancellations.add('events.refreshEventTimes', () => clearInterval(id));
          let request = client.events(namespace).asTable().watch(events_list.metadata.resourceVersion);
          if (object) {
            request = request.fieldSelector(`involvedObject.uid=${object}`);
          }
          const { promise, cancellation } = request.get({ generator: function* () { return yield* watchEventChanges(screen, page) } });
