...
class Events {
  ...
  constructor ({ screen, client, debug, navbar, namespace, status }) {
    ...
    events_table.on('select', (_, i) => {
      ...
      until(promise)
        ...
        .succeed(_ => event_details.setLabel(`Event {grey-fg}[${name}]{/grey-fg} {inverse}${event.cells[columns['Type']]}{/inverse}`))
    ...
    until(promise)
      .spin(s => events_table.setLabel(`${s} ${label}`))
      .succeed(_ => events_table.setLabel(`${label}`))
      .then(response => {
        ...
        sortEvents();
        updateEventsTable();
        const id = setInterval(refreshEventTimes, 1000);
        cancellations.add('events.refreshEventTimes', () => clearInterval(id));
        const { promise, cancellation } = client.events(namespace).asTable().watch(events_list.metadata.resourceVersion).get({ generator: watchEventChanges });
