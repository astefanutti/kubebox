...
class Events {
  constructor ({ screen, client, debug, cancellations, key, navbar, namespace }) {
    ...
    .then(_ =>{
      const { promise, cancellation } = client.event(namespace)
    ...
    this.describe_event = function(i) {
      ...
      const { promise, cancellation } = client.describe_event(namespace, name).get({ cancellable : true });
      cancellations.add(key, cancellation);
      until(promise)
      .spin(s => event_description.setLabel(`${s} Event`))
      .succeed(_ => event_description.setLabel(`Event ${events_table.rows[i][2]} {inverse}${events_table.rows[i][4]}{/inverse}`))
      .then(response => {
        const events_description = yaml.stringify(JSON.parse(response.body.toString('utf8')));
          ...
          switch (change.type) {
            ...
            case 'DELETED': {
              ...
              if (i < selected) {
                ...
                event_description.setLabel(`Describe {grey-fg}[${deleted_event[2]}]{/grey-fg} {red-fg}DELETED{/red-fg}`);
              }
