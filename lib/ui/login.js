...
function login_form(kube_config_manager, screen) {
  ...
  const cluster = blessed.textbox({
    ...
    left         : 15,
    top          : 0,
    value        : kube_config_manager.current_context.cluster.server
  });
  ...
  const username = blessed.textbox({
    ...
    left         : 15,
    top          : 2,
    value        : kube_config_manager.current_context.user.username
  });
  // retain key grabbing as text areas reset it after input reading
  ...
  const token = blessed.textbox({
    ...
    left         : 15,
    top          : 4,
    value        : kube_config_manager.current_context.user.token
  });
  // retain key grabbing as text areas reset it after input reading
...
function prompt(screen, kube_config_manager) {
  return new Promise(function(fulfill, reject) {
    screen.saveFocus();
    screen.grabKeys = true;
    const { form, username, password, token, cluster } = login_form(kube_config_manager, screen);
    screen.append(form);
    form.focusNext();
    screen.render();
