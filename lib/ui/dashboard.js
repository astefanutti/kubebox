...
class Dashboard {
  ...
  constructor(screen, client, debug) {
    ...
    pods_table.on('select', (item, i) => {
        ...
        delay(1000)
          ...
          .then(pod => {
            ...
            if (pod.metadata.deletionTimestamp) {
              ...
              const { promise, cancellation } = get(client.follow_log(namespace, name, timestamp), timestamp
                ? function*() {
      ...
      const { promise, cancellation } = get(client.follow_log(namespace, name), logger);
      cancellations.add('dashboard.logs', cancellation);
      promise
      ...
      pollStatsFromCAdvisor(pod).catch(error => console.error(error.stack));
      const id = setInterval(pod => pollStatsFromCAdvisor(pod).catch(error => console.error(error.stack)), 10000, pod);
    ...
    function pollStatsFromCAdvisor(pod) {
      let request;
      if (client.openshift) {
        request = get(client.container_stats(pod.spec.nodeName, pod.metadata.namespace, pod.metadata.name, pod.metadata.uid, pod.spec.containers[0].name))
          .then(response => response.body.toString('utf8'))
      } else {
        request = get(client.cadvisor_container_stats(pod.spec.nodeName, pod.status.containerStatuses[0].containerID.slice(9)))
          .then(response => response.body.toString('utf8'))
