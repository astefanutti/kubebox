...
class Dashboard {
  ...
  constructor({ screen, navbar, status, client, debug }) {
    ...
    pods_table.on('select', (item, i) => {
      ...
      const stats = updateStats(pod, container);
      ...
      until(stats.promise)
        ...
        .then(() => {
          ...
          const id = setInterval(pod => {
            const { promise, cancellation } = updateStats(pod, container);
        ...
        .catch(error => {
          // TODO: handle 404 with retry for empty historical metrics
          resources.setLabel('Resources');
          if (error.response) {
            let message;
    ...
    function updateStats(pod, container) {
      let promise, request;
      request = get(client.pod_metrics(pod.metadata.namespace, pod.metadata.name), { cancellable: true });
      promise = request.promise
        .then(response => response.body.toString('utf8'))
        .then(response => JSON.parse(response))
        .then(response => {
          const timestamps = response.items.map(s => moment(s.timestamp).format('HH:mm:ss'));
          const container_usage = response.items.flatMap(i => i.containers)
          ...
          const memory_usage = {
            title : 'Usage',
            x     : timestamps,
            y     : container_usage.map(u => parseInt(/(\d+)(Ki)?/.exec(u.memory)[1], 10) * 1024),
            style : { line: 'blue' },
          };
          const memory_stats = [memory_usage];
          if (false && container.resources.limits && container.resources.limits.memory) {
            const memory_limit = {
              title : 'Limit',
              x     : timestamps,
          ...
          const cpu_total = {
            title : 'Total',
            x     : timestamps,
            y     : container_usage.map(u => parseInt(/(\d+)(m)?/.exec(u.cpu)[1], 10)),
            style : { line: 'blue' },
          };
          ...
          if (false) {
            const cpu_limit = {
              title : 'Limit',
              x     : timestamps,
              y     : Array(timestamps.length).fill(response.spec.cpu.quota / response.spec.cpu.period),
              style : { line: 'red' },
            };
