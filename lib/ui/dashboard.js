...
class Dashboard {
  ...
  constructor({ screen, navbar, status, client, debug }) {
    ...
    function selectContainer(pod, container) {
      ...
      cancellations.add('dashboard.pod.stats', spec.cancellation);
      until(spec.promise
        .then(spec => {
          ...
          // get the stats
          const { promise, cancellation } = updateStatsFromCAdvisor(spec, cgroups, pod, container);
          cancellations.set('dashboard.pod.stats', cancellation);
          return promise;
        }))
        ...
        .then(({ spec, cgroups, pod, container }) => {
          ...
          let cancelled = false;
          let cancellation = () => {};
          cancellations.set('dashboard.pod.stats', () => {
            cancellation();
            cancelled = true;
          });
    ...
    function specFromCAdvisor(pod) {
      const request = client.cadvisor_spec(pod).get({ cancellable: true });
    ...
    function updateStatsFromCAdvisor(spec, cgroups, pod, container) {
      ...
      const request = client.cadvisor_stats(pod, cgroup.substring(0, cgroup.lastIndexOf('/'))).get({ cancellable: true });
      const promise = request.promise
        .then(response => {
          ...
          const cpu_user = {
            ...
            y     : containerStats.map(s => s.cpu.usage.user).delta().map((d, i) => d / 1e+6 / periods[i]),
            style : { line: 'cyan' },
          };
          const cpu_total = {
            title : 'Total',
            x     : timestamps.slice(1),
            y     : containerStats.map(s => s.cpu.usage.total).delta().map((d, i) => d / 1e+6 / periods[i]),
            style : { line: 'blue' },
          };
          ...
          // network
          const netStats = stats[cgroups['NET']];
          if (netStats && netStats.length > 0) {
            const net_periods = netStats.map(s => moment(s.timestamp).format('X')).delta();
            const net_timestamps = netStats.map(s => moment(s.timestamp).format('HH:mm:ss'));
            const net_rx = {
              title : 'RX',
              x     : net_timestamps.slice(1),
              // FIXME
              y     : netStats.map(s => s.network.interfaces[0].rx_bytes).delta().map((d, i) => d / net_periods[i]),
              style : { line: 'green' },
            };
            const net_tx = {
              title : 'TX',
              x     : net_timestamps.slice(1),
              // FIXME
              y     : netStats.map(s => s.network.interfaces[0].tx_bytes).delta().map((d, i) => d / net_periods[i]),
              style : { line: 'cyan' },
            };
