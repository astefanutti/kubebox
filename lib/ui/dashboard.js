...
class Dashboard {
  ...
  constructor({ screen, navbar, status, client, debug }) {
    ...
    pods_table.on('select', (item, i) => {
      ...
      // A container in a pod in Error or CrashLoopBackOff can be waiting, yet it is possible to query
      // log from previous runs
      if (k8s.isContainerWaitingWithReasons(pod, container, 'PodInitializing', 'ContainerCreating')) {
        pod_log.setLabel(`Logs {grey-fg}[${container.name}]{/grey-fg} {red-fg}POD INITIALIZING{/red-fg}`);
        // TODO: watch for the pod status and stream log once it's running
        return;
      }
