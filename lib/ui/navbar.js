...
class NavBar {
  ...
  constructor(screen) {
    ...
    function updateTabTitle(tab, index, name) {
      const prefix = index + 1 + '';
      const t = blessed.helpers.generateTags(tabs.style.prefix /* || { fg: 'lightblack' } */);
      const title = (prefix != null ? t.open + prefix + t.close + ':' : '') + name;
      const len = tab.strWidth((prefix != null ? prefix + ':' : '') + name);
    ...
    this.remove = function (selector) {
      ...
      tabs.commands.forEach((cmd, j) => {
        cmd.prefix = j + 1;
        updateTabTitle(cmd.element, j, items[j].name);
      });
      tabs.render();
