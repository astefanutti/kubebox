...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    screen.key(['e'], () => {
      ...
      const events_tab = new Events({ screen, client, debug, navbar, namespace: current_namespace, status });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
    ...
    screen.key(['C-e'], () => {
      ...
      const events_tab = new Events({ screen, client, debug, navbar, status });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
