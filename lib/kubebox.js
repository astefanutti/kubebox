...
class Kubebox {
  ...
  constructor(screen) {
    ...
    // FIXME: avoid 'n' key to open another prompt
    screen.key(['n'], () => {
      ...
      return get(client.get_apis())
        ...
        .catch(error => {
          ...
          if (error.response && (error.response.statusCode === 403 || error.response.statusCode === 401)) {
            ...
            return authenticate().then(connect);
          } else {
    ...
    function authenticate() {
      ...
      return (session.login ? Promise.resolve(session.login) : promptLogin())
        .then(updateSessionAfterLogin)
        .then(credentials => util.isEmpty(credentials.token)
          // try retrieving an OAuth access token from the OpenShift OAuth server
          ? get(os.platform() === 'browser' ? client.oauth_authorize_web(credentials) : client.oauth_authorize(credentials))
            .then(response => os.platform() === 'browser'
              ? response.body.toString('utf8').match(/<code>(.*)<\/code>/)[1]
