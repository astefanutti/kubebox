  ...
  constructor(screen, server) {
    ...
    function connect(login, options = {}) {
      ...
      return until(promise
        ...
        .then(() => {
          const { promise, cancellation } = client.paths({ authorization: !CORS }).get({ cancellable: true });
          cancellations.add('connect', cancel(cancellation));
          // try getting master API paths
          promise.then(response => client._paths = JSON.parse(response.body.toString('utf8')).paths)
