...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    screen.key(['e'], () => {
      const key = `Events ${current_namespace}`;
      if (navbar.select(key)) {
        return;
      }
      const events_tab = new Events({ screen, client, debug, cancellations, navbar, namespace: current_namespace, status });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
    ...
    screen.key(['C-e'], () => {
      const key = 'Events Cluster';
      if (navbar.select(key)) {
        return;
      }
      const events_tab = new Events({ screen, client, debug, cancellations, navbar, status });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
