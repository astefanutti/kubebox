...
const { Dashboard, Exec, login, namespaces, NavBar, spinner } = require('./ui/ui');
...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    dashboard.on('exec', (namespace, pod, container) => {
      ...
      const { promise, cancellation } = get(client.exec(namespace, pod, { container, command: ['/bin/sh', '-c', `TERM=${exec.termName()} sh`] }), { generator: () => exec.print(), readable: exec });
