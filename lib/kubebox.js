...
class Kubebox extends EventEmitter {
  ...
  constructor(screen, server) {
    ...
    screen.key(['e'], () => {
      const key = `${current_namespace} Events`;
      if (navbar.select(key)) {
        return;
      }
      const events_tab = new Events({ screen, client, debug, cancellations, key, navbar, namespace: current_namespace });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
    ...
    screen.key(['C-e'], () => {
      const key = 'Cluster Events';
      if (navbar.select(key)) {
        return;
      }
      const events_tab = new Events({ screen, client, debug, cancellations, key, navbar });
      navbar.add({
        name   : key,
        render : _ => {
          events_tab.render();
