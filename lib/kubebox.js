...
const login             = require('./ui/login'),
      ...
      KubeConfigManager = require('./config/kube-config-manager');
...
class Kubebox {
  ...
  constructor(screen) {
    ...
    const kube_config_manager = new KubeConfigManager();
    const client = new Client();
    client.master_api = kube_config_manager.current_context.getMasterApi();
    session.namespace = kube_config_manager.current_context.namespace.name;
    ...
    function logging() {
      return login.prompt(screen, kube_config_manager)
    ...
    function updateSessionAfterLogin(login) {
      kube_config_manager.updateOrInsertContext(login);
      client.master_api = kube_config_manager.current_context.getMasterApi();
      session.namespace = kube_config_manager.current_context.namespace.name;
      return login;
    }
