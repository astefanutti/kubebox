...
class ListRequest extends ApiRequest {
  ...
  // handle continued request if needed
  limit(limit) {
...
class LimitListRequest extends ListRequest {

  watch(_) {
    throw Error('List requests with a limit do not support watch for changes');
  }
}

class WatchListRequest extends ListRequest {

  limit(_) {
    throw Error('List requests with a watch do not support limit');
  }
}
...
class Client {
  ...
  events(namespace) {
    return new ListRequest({ path: namespaced`/api/v1/events?limit=500${namespace}` }, { client: this });
  }
