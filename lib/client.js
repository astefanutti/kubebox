...
const crypto     = require('crypto'),
      ...
      namespaced = require('./util').namespaced,
      URI        = require('urijs');
...
class Client {
  ...
  events(namespace) {
    return new WatchableRequest({ path: namespaced`/api/v1/events?limit=500${namespace}`, headers: { 'Accept' : 'application/json;as=Table;v=v1beta1;g=meta.k8s.io, application/json' }}, { client: this });
  }

  event(namespace, name) {
    return new ApiRequest({ path: `/api/v1/namespaces/${namespace}/events/${name}`, headers: { 'Accept' : 'application/yaml;v=v1beta1;g=meta.k8s.io, application/yaml' } }, { client: this });
  }
